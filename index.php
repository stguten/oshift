...
foreach($linhas as $l){
	...
	try{
		...
		for($i = 0; $i < count($xxx);$i++){
			print_r("[".$xxx[$i]["codVehicle"]."]".$xxx[$i]["patternName"].": ".$xxx[$i]["arrivalTime"]." minutos.(<a href=\"https://www.google.com/maps/place/".$xxx[$i]["latLng"]["lat"].",+".$xxx[$i]["latLng"]["lng"]."\">Localização no mapa</a>)<br>");
		}
		...
		for($i = 0; $i < count($xxx2);$i++){
			print_r("[".$xxx2[$i]["codVehicle"]."]".$xxx2[$i]["patternName"].": ".$xxx2[$i]["arrivalTime"]." minutos.(<a href=\"https://www.google.com/maps/place/".$xxx2[$i]["latLng"]["lat"].",+".$xxx2[$i]["latLng"]["lng"]."\">Localização no mapa</a>)<br>");
		}
	}catch(Exception $e){
